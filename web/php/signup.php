...
if(!empty($_POST['Username']) && !empty($_POST['Firstname']) && !empty($_POST['Password'])){
	...
	if(mysqli_num_rows($result) == 0){
		...
		{	
			// $var = "studentinfo";
			$query = "INSERT INTO $var (`username`, `firstname`, `lastname`, `email`, `image_url`, `credential`, `description`, `cgpa`)
			 VALUES('$username', '$firstname','$lastname','','fb_avatar_male.jpg','','','0')";
			// echo $query;
		}
		elseif($profession == 'faculty')
		{
			// $var = "facultyinfo";
			$query = "INSERT INTO $var (`username`, `firstname`, `lastname`, `email`, `image_url`, `credential`, `description`) 
			VALUES('$username', '$firstname','$lastname','','fb_avatar_male.jpg','','')";
		}
		$result = mysqli_query($dbc, $query)
